...
上面代码是`package.json`文件的一个片段，里面的`scripts`字段是一个对象。它的每一个属性，对应一段脚本。比如，上面的`build`命令，对应的脚本是`node build.js`。这段脚本可以是任何的 Shell 命令。

然后，在命令行下使用`npm run`命令，就可以执行这段脚本。
...
这些定义在`package.json`里面的脚本，就是 npm 脚本。它的优点很多。

> - 项目相关的脚本，可以集中在一个地方。
- 不同的脚本命令，可以有同样的对外接口。
- 可以利用 npm 提供很多辅助功能。

如果要查看当前项目的所有脚本命令，可以使用不带任何参数的`npm run`命令。
...
npm 脚本的原理非常简单。每当执行`npm run`，就会自动新建一个 Shell，在这个 Shell 里面执行指定的脚本命令。因此，只要当前的默认 Shell（一般是 Bash）可以运行的命令，就可以写在 npm 脚本里面。
...
上面代码中，如果用户执行`npm run start`，默认执行`node server.js`，前提是项目根目录下有`server.js`这个脚本。如果用户执行`npm run install`，默认执行`node-gyp rebuild`，前提是项目根目录下有`binding.gyp`文件。
...
它们的作用是，用户执行`npm run build`的时候，会自动按照下面的顺序执行。

```bash
npm run prebuild && npm run build && npm run postbuild
```

因此，可以在这两个钩子里面，完成一些准备工作和清理工作。下面是另一个例子。
...
`npm_package_`前缀支持嵌套的`package.json`字段。
...
如果要向 npm 脚本传入参数，要使用`--`标明。

```javascript
"lint": "jshint **.js"
```

要向上面的`npm run lint`命令传入参数，必须写成下面这样。
