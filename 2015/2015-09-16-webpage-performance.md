...
这种网页响应非常缓慢，占用大量的CPU和内存，浏览起来常常有卡顿，页面的动画效果也不流畅。毫无疑问，这时你就会关闭它，改为访问其他网站。

作为一个开发者，肯定是不愿意看到这种情况的，那么怎么才能提高网页的性能呢？

本文介绍网页性能问题的原因，及其解决方法。
...
我们常常把“生成布局”（flow）和“绘制”（paint）这两步，合称为“渲染”（render）。
...
需要注意的是，某些时候重新渲染可能不需要“重排”，直接就“重绘”了。比如，改变某个网页元素的颜色，就只会触发“重绘”，不会触发“重排”，因为布局没有改变。

但是，如果改变一个网页元素的位置，就会同时触发“重排”和“重绘”，因为布局改变了。对于浏览器来说，只要有“重排”，就必然会触发“重绘”。

## 三、对于性能的影响

重排和重绘会不断触发，这是不可避免的。但是，它们非常耗费资源，是导致网页性能低下的根本原因。

**我们要做的，就是降低它们的频率和成本，尽量少触发重新渲染。**

前面提到，DOM变动和样式变动，都会触发重新渲染。浏览器会尽量把所有的变动，集中在一起，排成一个队列，然后一次性执行。
...
上面代码对div元素有两个变动，但是只会触发一次重排和重绘。

但是，如果写得不好，就会触发两次重排和重绘。
...
上面代码对div元素设置背景色以后，要求浏览器给出该元素的位置，所以浏览器不得不立即重排。

一般来说，样式的写操作之后，如果有下面这些属性的读操作，都会引发浏览器立即重绘。
...
（2）如果某个样式是通过计算得到的，那么最好缓存计算结果。避免下一次用到的时候，浏览器重新计算。
...
（4）使用离线DOM，而不是实际的DOM，来改变元素样式。

最好的例子就是把多个DOM操作，放在一个Document Fragment对象上面，操作完成后，再把这个对象加入DOM。

另外一个例子是使用 cloneNode() 方法，在克隆的节点上进行操作，然后再把克隆的节点替换原始节点。

还有一种方法是，先将元素设为 display: none （1次重排和重绘），然后对这个节点进行100次操作，最后再恢复显示（1次重排和重绘）。这样一来，你就用两次重新渲染，取代了可能高达100次的重新渲染。

（5）position属性为absolute或fixed的元素，重排的开销会比较小，因为不用考虑它对其他元素的影响。

（6）只在必要的时候，才将元素的display属性为可见，因为不可见的元素不影响重排和重绘。

另外，visibility : hidden 的元素只对重排有影响，不影响重绘。

（7）采用虚拟DOM的脚本库，比如React等。

（8）使用 window.requestAnimationFrame()、window.requestIdleCallback() 这两个方法（详见后文）。

## 五、刷新率

很多时候，密集的重新渲染是无法避免的，比如scroll事件的回调函数和网页动画。

网页动画的每一帧（frame）都是一次重新渲染。每秒低于24帧的动画，人眼就能感受到停顿。如果能达到每秒60帧甚至80帧，就会非常流畅。

由于大多数显示器的刷新频率是60Hz，浏览器内部会将动画按照这个频率同步刷新（如果可以做到的话）。所以如果网页动画能够做到每秒60帧，就会跟显示器同步刷新，达到最佳的视觉效果。这意味着，一秒之内进行60次重新渲染，每次重新渲染的时间不能超过16.66毫秒。
