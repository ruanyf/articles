...
这个重要的 CSS 新功能，已经可以在所有主要浏览器使用了。本文将全面介绍如何使用它，你会发现原生 CSS 从此变得异常强大。

## 一、变量的声明

CSS 变量（CSS variable）的正式名称，叫做“CSS 自定义属性”（CSS custom properties）。因为，变量与自定义的 CSS 属性其实是一回事。
...
上面代码中，`body`选择器里面声明了两个变量：`--foo`和`--bar`。它们与`color`、`font-size`等正式属性没有什么不同，只是浏览器不会去处理它们，所以叫做“自定义属性”。

你可能会问，为什么选择两根连词线（`--`）表示变量？

回答是`$foo`被 Sass 用掉了，`@foo`被 Less 用掉了。为了不产生冲突，官方的 CSS 变量就改用两根连词线了。 
...
变量名是大小写敏感的，`--header-color`和`--Header-Color`属于两个不同变量。
...
上面代码中，数值与单位直接写在一起，这是无效的。只能使用`calc()`函数，将它们连接。
...
同一个 CSS 变量，可以在多个选择器内声明。读取的时候，哪个选择器优先级最高，它内部的声明就会生效。这与 CSS 的“层叠”（cascade）规则是一致的。
...
从作用域的角度来看，变量的作用域就是它所在的选择器生效的范围。
...
CSS 变量的声明是动态的。这个特点可以用在响应式布局之中，不同的屏幕宽度使用不同的变量值。
