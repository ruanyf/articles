...
但是，Bash 的语法很独特，大量的细节需要学习。

![](http://www.ruanyifeng.com/blogimg/asset/2017/bg2017111501.png)

本文介绍`set`命令的用法，它很重要，所有服务端工程师都应该掌握。
...
`set`命令就是用来修改 Shell 环境的运行参数。后面介绍其中四个最常用的参数，完整的参数清单可以查看[官方手册](https://www.gnu.org/software/bash/manual/html_node/The-Set-Builtin.html)，
...
脚本可以使用变量，如果遇到不存在的变量，Bash 默认忽略它。
...
可以看到，`echo $a`输出了一个空行，然后 Bash 继续执行`echo bar`。大多数情况下，这不是开发者想要的行为，遇到变量不存在，脚本应该报错，而不是一声不响地往下执行。

`set -u`就用来改变这种行为。脚本头部加上它，然后遇到不存在的变量就会报错，并停止执行。
