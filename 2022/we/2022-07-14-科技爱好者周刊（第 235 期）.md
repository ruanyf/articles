...
1、[E-ink 手机显示器](https://liliputing.com/dasung-link-is-a-6-7-inch-e-ink-display-that-pairs-with-your-smartphone/)
...
它等于为手机增加了第二块屏幕，同时又很省电，适合专门用来阅读。

1、[B-21 轰炸机的位置](https://threadreaderapp.com/thread/1600683623250030593.html)
...
军方原始上传的照片带有拍摄时间，因此根据照片上星座与地平线的距离，判断地点在北纬34度到35度之间。

美国本土在这个纬度的空军基地并不多，距离制造公司最近的就是加州的爱德华兹空军基地。

![](https://cdn.beekka.com/blogimg/asset/202212/bg2022120911.webp)

这个基地本来就停放轰炸机，谷歌地图甚至有 B-2 轰炸机在该基地的卫星照片（下图）。将机库建筑与照片比对，甚至可以确定 B-21 的具体停放位置。
...
1、[gpt-commit-summarizer](https://github.com/KanHarI/gpt-commit-summarizer)

一个 GitHub Action 脚本，用户需要有 OpenAI API 密钥。当你向仓库提交代码时，它会使用 AI 分析，自动生成本次提交的主要内容，参见[介绍文章](https://medium.com/@knaan.harpaz/leverage-openais-language-model-for-automated-commit-summaries-8181cef30375)。
...
一个专门为程序员开发的搜索引擎，可以用自然语言输入你的问题，比如“使用 PHP 的 hello world 示例”，它会直接给出答案。
