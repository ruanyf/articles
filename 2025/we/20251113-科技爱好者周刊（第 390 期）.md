# 科技爱好者周刊（第 390 期）

## 封面图

![](https://cdn.beekka.com/blogimg/asset/202603/bg2026032201.webp)

山东日照某住宅小区的彩色风雨连廊，入口处树林还设了咖啡馆。（[via](https://mooool.com/coffee-tree-canopy-by-selgascano.html)）

## AI 的贫富分化

工业革命的一个后果，就是实现了“消费者平等”。

在消费品面前，穷人和富人是平等的，大家喝一样的可口可乐，用一样的苹果手机，开一样的特斯拉。

但是，AI 模型并非如此。

**在大模型面前，穷人和富人是不平等的**。未来的普通人几乎肯定用不起顶级的大模型。

其实，现在就是这样了。目前最贵的 AI 编程套餐是 Claude Code 的 Max 套餐，月费200美元，很多人已经用不起了。

更不用说，OpenAI 曾经设想过[月费2万美元的套餐](https://www.thepaper.cn/newsDetail_forward_30320495)，专供顶级大模型服务。

![](https://cdn.beekka.com/blogimg/asset/202603/bg2026032207.webp)

要是真的推出，只有富豪才用得起。

这说明了一个不争的事实，**费用越贵，模型效果越好**。

原因很简单，大模型的效果与算力相关。更多的算力、更大的上下文、更多的参数，都需要钱。

这跟工业品完全相反。富人有钱，也买不到超级 iPhone，但可以买到超级模型。

工业品有规模效应，产量越高，单位成本越低，所以一旦大规模生产，最好的产品会变得便宜；**大模型不存在规模效应**，多用几台服务器，并不会让单位成本降低，反而可能因为扩建机房，改造电路和水路，而变得更贵。

所以，未来有两种可能。

一种可能是贫富分化，富人和穷人用不同的模型，有钱就享受顶级模型的规划、咨询、生成服务，没钱就只能用免费模型，效果非常普通。

另一种是算力变得足够便宜，所有人都用最好的模型。就像马斯克[最近说的](https://wap.cj.sina.cn/7x24/4762771)，算力供应最终会无限大。

![](https://cdn.beekka.com/blogimg/asset/202603/bg2026032206.webp)

不过我还是觉得，前一种可能更现实。

https://newsletter.danielpaleka.com/p/you-are-going-to-get-priced-out-of

## 大模型并不神奇 没有语料，大模型就是智障

现在，很多人把大模型当作很神奇的东西，推崇备至。

当然，它确实很神奇，但并不是魔法，而是基于统计学规律的语言模型，是可以解释的。

证据就是，**如果让它解没有训练过的题目，它根本解不出来**。

两位国外的研究者做了一个实验，找了五个主流模型，让它们用五种小众的编程语言（Brainfuck、Befunge-98、Whitespace、Unlambda 和 Shakespeare）解题。

![](https://cdn.beekka.com/blogimg/asset/202603/bg2026032008.webp)

他们把实验结果写成[论文](https://esolang-bench.vercel.app/)（上图），就是一句话：大模型的表现一塌糊涂。

这五种模型的平均正确率，仅为3.8%，就是**100道题可以答对3.8道**。相比之下，它们处理 Python 问题的正确率可以达到90%。

更尴尬的是，题目的难度分成四级：入门、初级、中级、高级。除了入门级，大模型在其他三个级别的正确率是0。

这充分说明了，模型的表现首先由训练材料决定：如果训练的语料少，模型就用处不大。反之，语料像 Python 语言那样多到遍地都是，模型就会天下无敌。

我好奇的是，如果某种冷门语言没有语料，但有一本很详尽的《使用手册》，我让大模型学习这本手册，它是否就能用这种冷门语言编程了。

## MAI-Image-2

本周，微软公司发布了自家的图像生成模型 [MAI-Image-2](https://microsoft.ai/news/introducing-MAI-Image-2/)。

这个模型的图像质量非常高，有[评论](https://decrypt.co/361791/microsoft-mai-image-2-text-image-model-review)认为，目前仅次于谷歌的 nano-banana-2。

微软开放了网站 [MAI Playground](https://playground.microsoft.ai/chat)（下图），可以免费生成图片。

![](https://cdn.beekka.com/blogimg/asset/202603/bg2026032009.webp)

我试用后，图像质感很好，很逼真。比如，一只狗在海里骑自行车。

![](https://cdn.beekka.com/blogimg/asset/202603/bg2026032010.webp)

但是，它的限制很多，有争议的、可能冒犯的图片，它都拒绝生成。每天的免费额度是15张，每次生成的间隔时间是30秒。而且，它只能生成长宽比 1:1 的图片，其他分辨率都不支持，也不提供图像编辑和加工。

## 科技动态

1、[可玩的封面](https://www.yankodesign.com/2026/03/16/red-bull-just-put-a-playable-tetris-game-on-a-magazine-cover/)

红牛公司推出了一本游戏杂志《GamePop》，纸质印刷。

![](https://cdn.beekka.com/blogimg/asset/202603/bg2026032002.webp)

它的封面有一个可以玩的“俄罗斯方块”，是世界首款可以玩游戏的书籍封面。

![](https://cdn.beekka.com/blogimg/asset/202603/bg2026032003.webp)

![](https://cdn.beekka.com/blogimg/asset/202603/bg2026032004.webp)

它的奥秘就是封面嵌入了一块非常薄的柔性电路板。

![](https://cdn.beekka.com/blogimg/asset/202603/bg2026032005.webp)

这块板配备了180个 RGB LED 灯、7个电容式触摸按钮、一颗32位 ARM 芯片，以及一块比头发丝还细的柔性电路板。

![](https://cdn.beekka.com/blogimg/asset/202603/bg2026032006.webp)

它还包含一块可充电电池，可以通过 Type C 充电。

![](https://cdn.beekka.com/blogimg/asset/202603/bg2026032007.webp)

可惜的是，这个封面是限量版，得到了俄罗斯方块公司的官方授权，全球仅发行150套，每套均有独立编号。

1、[收费的真人客服](https://arstechnica.com/gadgets/2025/02/misguided-hp-customer-support-approach-included-forced-15-minute-call-wait-times/)

对于企业来说，真人的客服电话成本很高，最好改成机器应答。

一年前，惠普公司在欧洲推行改革。用户打客服电话，会听到语音提示，要求改为访问客服网站，如果你坚持要真人客服，需要在线等待15分钟。

![](https://cdn.beekka.com/blogimg/asset/202603/bg2026032203.webp)

在这段时候，系统会时不时提示“不要挂断”，还分别在通话的第5、10和13分钟提醒你可以访问网站或邮件联系。15分钟等待结束后，才会接入真人客服。

以后这种做法可能是常规，普通用户只能接受 AI 客服或机器人客服，想要真人客服，必须额外付费。

1、[飞盘的玩法](https://arstechnica.com/science/2024/10/how-physics-can-improve-your-disc-golf-game/)

怎样扔飞盘，才能又快又远？

一个美国的物理学家，找了几十个学生进行实验，测量不同手势和角度，扔出飞盘的速度和扭矩。他把结果写成论文发表。

![](https://cdn.beekka.com/blogimg/asset/202411/bg2024110510.webp)

他发现，将拇指放置在距飞盘外缘约3厘米的位置，对于所有参与者来说，可以同时获得平均转速和初始速度的最佳结果。

![](https://cdn.beekka.com/blogimg/asset/202411/bg2024110511.webp)

转速与初始速度之间存在线性相关，转速越高，初始速度也越高。

所以，大家下次玩飞盘，要放对拇指的位置，然后用足力气，反手扔出，就可以取得最佳结果。

1、一个荷兰设计师发明了家用的[风力编织机](https://www.merelkarhof.nl/work/wind-knitting-factory)，可以安装在阳台或楼顶。

![](https://cdn.beekka.com/blogimg/asset/202507/bg2025070405.webp)

风力带动叶片，在一个圆形的编制转盘上，将毛线编织成围巾。

![](https://cdn.beekka.com/blogimg/asset/202507/bg2025070406.webp)

风大的时候，编织速度非常快。它看上去是一个纯粹的机械装置，没有用到风力发电。

![](https://cdn.beekka.com/blogimg/asset/202507/bg2025070407.webp)

设计师的原意是向人们展示，在城市的楼宇里，也可以利用风力。

![](https://cdn.beekka.com/blogimg/asset/202507/bg2025070408.webp)


1、[树的生长](https://www.instagram.com/p/DN3btHw0MKn/)

如果你的手一直握着一颗树，会怎样？

![](https://cdn.beekka.com/blogimg/asset/202601/bg2026010301.webp)

一个意大利艺术家做了一个实验。2004年，他用青铜做了一只手的模型，握在一颗树上。

![](https://cdn.beekka.com/blogimg/asset/202601/bg2026010302.webp)

![](https://cdn.beekka.com/blogimg/asset/202601/bg2026010303.webp)

结果，整棵树继续长，除了手握着的那个地方。

![](https://cdn.beekka.com/blogimg/asset/202601/bg2026010304.webp)

![](https://cdn.beekka.com/blogimg/asset/202601/bg2026010305.webp)

到了2018年，手已经陷入树里了，树在手握的地方空出了一块。

![](https://cdn.beekka.com/blogimg/asset/202601/bg2026010306.webp)

1、[送货机器人占据人行道](https://blockclubchicago.org/2025/12/08/delivery-robots-take-over-chicago-sidewalks-sparking-debate-and-a-petition-to-hit-pause/)

美国大城市芝加哥，大量使用送货机器人，已经引起了居民抗议。

![](https://cdn.beekka.com/blogimg/asset/202512/bg2025120917.webp)

有人认为，这些机器人占据了人行道，妨碍了行人。

![](https://cdn.beekka.com/blogimg/asset/202512/bg2025120918.webp)

一位市民说，“人行道是为人准备的，不是为送货机器人准备的。安静的居民区人行道上，出现几十辆甚至上百辆这样的车，将会变成什么样子？”

![](https://cdn.beekka.com/blogimg/asset/202512/bg2025120919.webp)

![](https://cdn.beekka.com/blogimg/asset/202512/bg2025120920.webp)

1、一家美国的飞机制造商，推出[新型的私人飞机](https://www.wsj.com/business/airlines/this-windowless-plane-is-vying-to-be-the-private-jet-of-the-future-2fdf184b?st=fHNiyn&mod=1440&user_id=66c4c9305d78644b3ac5df9c)，最大特点是没有窗户。

![](https://cdn.beekka.com/blogimg/asset/202510/bg2025100210.webp)

它的内部都是显示屏，播放飞机外部的高分辨率摄像头拍到的景象。

该公司表示，没有窗户可以让飞机更轻，外表更光滑减少空气阻力，减少燃油消耗。而且，乘客不必探头，就能在座位上环顾空中的整个视野，而不是原来小窗口的视野。

## 文章

1、[下一个 App 很可能是无头应用](https://tuananh.net/2026/03/18/why-your-next-mobile-app-is-probably-headless/)（英文）

![](https://cdn.beekka.com/blogimg/asset/202603/bg2026032202.webp)

如果我们未来都通过 AI 助手使用手机，那么各种 App 就不需要显示模块（无头），只需提供数据接口。

1、[网页前端数据压缩的一种方法](https://jstrieb.github.io/posts/canvas-compress/)（英文）

![](https://cdn.beekka.com/blogimg/asset/202602/bg2026022105.webp)

本文介绍如何在前端，通过 canvas（画布）将数据压缩成一张图片。

1、[罗马人的建筑](https://unchartedterritories.tomaspueyo.com/p/how-roman-technology-drove-its-architecture)（英文）

![](https://cdn.beekka.com/blogimg/asset/202603/bg2026032011.webp)

出于巧合，罗马人发现了混凝土，还发现了如何用它浇筑。结果就是，罗马建筑有古代最大的室内面积，并且非常坚固，保留到了今天。

1、[Ruby 是构建 AI 应用的最佳语言](https://paolino.me/ruby-is-the-best-language-for-ai-apps/)（英文）

![](https://cdn.beekka.com/blogimg/asset/202602/bg2026022406.webp)

作者比较了 Python、JavaScript、Ruby 三种语言写一个跟 AI 模型交互的程序，认为 Ruby 写 AI 应用最方便。

1、[负载均衡的健康检查](https://singh-sanjay.com/2026/01/12/health-checks-client-vs-server-side-lb.html)（英文）

![](https://cdn.beekka.com/blogimg/asset/202602/bg2026022403.webp)

本文介绍了负载均衡可以做在服务器，也可以做在客户端，这两种情况下如何检查有故障的节点。

1、[天空为何是蓝色的](https://explainers.blog/posts/why-is-the-sky-blue/)（英文）

![](https://cdn.beekka.com/blogimg/asset/202602/bg2026021616.webp)

本文通俗解释了，跟天空颜色相关的一系列问题：为何天空是蓝色，日落时却是红色，为何云是白色的，为何火星上天空是红色的，日落是蓝色的。

1、[HTML 替代 JS 的四种场景](https://www.htmhell.dev/adventcalendar/2025/27/)（英文）

![](https://cdn.beekka.com/blogimg/asset/202512/bg2025122902.webp)

本文提出，HTML + CSS 已经足够强，很多场景可以不用 JS，只用 HTML 实现，比如弹框和浮层。

1、[使用自定义 HTML 元素](https://maurycyz.com/misc/make-up-tags/)（英文）

![](https://cdn.beekka.com/blogimg/asset/202512/bg2025122903.webp)

网页完全可以用自定义的 HTML 元素，取代`div`，提供更好的语义。

1、[网页如何链接电话号码](https://sethmlarson.dev/mobile-browsers-and-telephone-numbers)（英文）

![](https://cdn.beekka.com/blogimg/asset/202511/bg2025112601.webp)

手机浏览器打开网页时，遇到网页里面的电话号码，会自动添加链接，点击可以拨打。本文教你怎么定制这个行为，包括取消链接，或者点击拨打其他号码。

1、[挑战者深渊有多深](https://storymaps.arcgis.com/stories/0d389600f3464e3185a84c199f04e859)（英文）

![](https://cdn.beekka.com/blogimg/asset/202511/bg2025112404.webp)

一篇图文介绍，用生动的图片解释地球最深处“挑战者深渊”（Challenger Deep），深达约11000米。

## 工具

4、[Valdi](https://github.com/Snapchat/Valdi)

Snap 公司发布的 UI 框架，可以用类似 React 的语法编写组件，然后编译成 iOS、Android 和 macOS 的原生应用。

3、[Npflared](https://npflared.thomas-cogez.fr/index.html)

![](https://cdn.beekka.com/blogimg/asset/202412/bg2024122502.webp)

架设私有 NPM 镜像的工具，适合放置内部 JS 软件包。

1、[Chokidar](https://github.com/paulmillr/chokidar)

一个 Node.js 模块，用来监听文件系统的各种事件（新增、删除、编辑等），比原生的 fs.watch / fs.watchFile 功能强。

1、[Speech Note](https://github.com/mkiol/dsnote)

![](https://cdn.beekka.com/blogimg/asset/202409/bg2024091301.webp)

Linux 桌面软件，可以将讲座音频转录成文字，并翻译成其他语言。

1、[Rustpad](https://github.com/ekzhang/rustpad)

![](https://cdn.beekka.com/blogimg/asset/202409/bg2024091810.webp)

一个开源的在线文本编辑器，支持多人在网页上实时编辑同一个文档。

1、[Speed-Test](https://github.com/openspeedtest/Speed-Test)

![](https://cdn.beekka.com/blogimg/asset/202312/bg2023121002.webp)

一个开源的测试网速工具，支持所有平台。由于可以自己部署，因此可以测试内网速度。

1、[Wave Terminal](https://www.waveterm.dev/)

![](https://cdn.beekka.com/blogimg/asset/202403/bg2024031202.webp)

一个现代感的、开源的终端模拟器，支持 macOS 和 Linux。

1、[Ayllu](https://ayllu-forge.org/)

![](https://cdn.beekka.com/blogimg/asset/202312/bg2023121101.webp)

Git 代码仓库的一个 Web UI，用于自己托管 Git 仓库。

1、[Don't Fuck With Paste](https://github.com/aaronraimist/DontFuckWithPaste)

![](https://cdn.beekka.com/blogimg/asset/202403/bg2024031006.webp)

一个开源的浏览器插件，可以让那些禁止复制粘贴的输入框，变得可以复制粘贴。

9、[Timelock](https://timelock.dev/)

![](https://cdn.beekka.com/blogimg/asset/202403/bg2024031201.webp)

这个开源的在线工具，用来加密消息，只有到了指定时间才能解密。

1、[proxychains-rs](https://github.com/tianrking/proxychains-rs)

proxychains4 的 Rust 实现，支持跨平台“指定进程走代理链”。（[@tianrking](https://github.com/ruanyf/weekly/issues/9316) 投稿）

1、[Flare Stack Blog](https://github.com/du2333/flare-stack-blog)

![](https://cdn.beekka.com/blogimg/asset/202603/bg2026032001.webp)

一个基于 Cloudflare Worker 的博客 CMS，集成 D1、R2、KV、Workflow 等服务。（[@du2333](https://github.com/ruanyf/weekly/issues/9312) 投稿）

1、[Tunelo](https://github.com/jiweiyuan/tunelo)

一行命令将本地服务暴露到公网，单个 4MB 二进制文件，QUIC 协议。（[@jiweiyuan](https://github.com/ruanyf/weekly/issues/9328) 投稿）

## AI 相关

1、[Lightpanda Browser](https://github.com/lightpanda-io/browser)

专门供 AI 自动化使用的无头浏览器，内存占用据称比 Chrome 小9倍。

1、[AI 星座](https://github.com/baturyilmaz/what-if-ai-agents-had-zodiac-personalities)

一个玩笑项目，让 AI 模拟12星座人格，然后看看不同星座对同一个事件如何反应。

1、[SurfSense](https://github.com/MODSetter/SurfSense)

![](https://cdn.beekka.com/blogimg/asset/202508/bg2025083009.webp)

AI 笔记本，谷歌 NotebookLM 的开源替代品。

1、[chat.md](https://github.com/rusiaaman/chat.md)

![](https://cdn.beekka.com/blogimg/asset/202508/bg2025083007.webp)

一个 VS Code 插件，将 AI 对话框变成一个 Markdown 文件，用户的输入和 AI 的输出都自动显示在这个文件。

3、[Parse](https://www.parse.bot/)

![](https://cdn.beekka.com/blogimg/asset/202508/bg2025081010.webp)

该网站让你使用自然语言，抓取其他网站的数据，转为 API 输出，目前好像是免费使用。

1、[Abogen](https://github.com/denizsafak/abogen)

![](https://cdn.beekka.com/blogimg/asset/202508/bg2025081106.webp)

一个跨平台的桌面应用，将 ePub/PDF/text 文件转成音频。

## 资源

1、[用于数据分析的 AI Coding Agent](https://simonw.github.io/nicar-2026-coding-agents/index.html)（英文）

![](https://cdn.beekka.com/blogimg/asset/202603/bg2026032204.webp)

著名开发者 Simon Willison 的培训班讲课资料，通过 AI 工具进行数据分析，有详细步骤。

1、[TypeScript 简洁之书](https://gibbok.github.io/typescript-book/zh-cn/book/the-concise-typescript-book/)

![](https://cdn.beekka.com/blogimg/asset/202601/bg2026011203.webp)

一本开源的 TypeScript 教程，有中文版。

1、[Docker 轻松上手](https://learn-how-docker-works.vercel.app/)

![](https://cdn.beekka.com/blogimg/asset/202601/bg2026011401.webp)

一个开源的互动式 Docker 入门网页教程（Docker made easy）。

1、[Bash Screensavers](https://github.com/attogram/bash-screensavers)

![](https://cdn.beekka.com/blogimg/asset/202510/bg2025103004.webp)

这个工具收集了十几种可以在 Bash 窗口运行的屏保效果。

1、[100本影响时代的科学著作](https://web.mnstate.edu/schwartz/centurylist2.html)

![](https://cdn.beekka.com/blogimg/asset/202502/bg2025020501.webp)

《美国科学家》杂志在1999年评出的100本最佳科学著作。

1、[Totally Useless Websites](https://www.totallyuselesswebsites.com)

![](https://cdn.beekka.com/blogimg/asset/202505/bg2025051206.webp)

该网站专门收集完全无用无意义的网站。

## 图片

1、[儿童死亡率](https://kottke.org/25/12/an-astonishing-graph)

现代人很难想象的是，人类历史的大部分时间里，儿童死亡率（在成年前死亡）一直接近50%。

![](https://cdn.beekka.com/blogimg/asset/202512/bg2025120307.webp)

工业革命开始以后，这个数字开始快速下降。到了2020年，全球平均儿童死亡率是4.3%，最低的国家已经达到了0.3%。

1、[地图里面的插图](https://eyeondesign.aiga.org/for-decades-cartographers-have-been-hiding-covert-illustrations-inside-of-switzerlands-official-maps/)

以前的地图都是手绘的，这是一项非常精细、枯燥的工作。

瑞士地图局（Swisstopo）的制图师，可能为了排遣无聊的时光，在官方地图里面居然插入一些隐蔽的插图，多年来都无人发现。

土拨鼠

![](https://cdn.beekka.com/blogimg/asset/202512/bg2025120210.webp)

蜘蛛

![](https://cdn.beekka.com/blogimg/asset/202512/bg2025120211.webp)

鱼

![](https://cdn.beekka.com/blogimg/asset/202512/bg2025120212.webp)

登山者

![](https://cdn.beekka.com/blogimg/asset/202512/bg2025120213.webp)

## 文摘

1、[别变成机器](https://blog.armeet.ca/becoming-the-machine/)

我最近看到一句话：“只有奴隶才通过生产力来量化自身的存在价值。”

这让我想到，如今的社交媒体上充斥着大量的奋斗文化的宣传，许多人展示自己是多么努力在提高个人生产力。

在我看来，这就是将自己跟机器做比较。大家认定，如果能把自己变成一台接收指令并持续朝着某个目标运转的机器，就能取得成功。

以下都是“奋斗文化”的表现：（1）你还不够努力。（2）你得早上五点起床。（3）你得第一个到，最后一个走。

这种文化的背后，是要求人变成机器。

但事实是，机器无法适应环境变化，也无法学习游戏规则。它是决定论的，以既定的模式和线性速度运转。

你不是机器，你是人。你应该力求灵活应变，快速适应。你不要追求极致的努力，而是要找到最有价值的解决方案。你要专注于真正重要的因素：速度、效率或质量……不要迷恋枯燥乏味的工作。

1、[软件工程师何时退休](https://thecodist.com/how-to-know-when-its-time-to-go/)

经过一年的考虑，我决定离开程序员岗位，退休了。

退休的原因不是能力有问题，而是我不想再干下去了。

每个人最终都会达到一个临界点，无法再做他们一生都在做的事情。这跟年龄无关，我认识比我年轻得多的人，他们也放弃了程序员职业。

我见过的退休原因有下面几种。

（1）能力不足。你再也完不成分配给你的任务，你的能力不适合行业的需求。

（2）缺乏欲望，对这个行业丧失了兴趣。

（3）就业市场不好，或者雇主倒闭，你找不到下一份工作。

（4）技术过时，你的技能无人需要。

（5）你发现其他值得做的事情。

（6）你赚了很多钱，然后觉得筋疲力尽，没有足够动力了，发现自己不再关心手头的事情。

所有程序员最终都会因为上述原因之一，而放弃程序员工作。

我也见过一些人，他们看重薪水，只要有工资，不管是否喜欢，都会一直做下去。这也是一种选择，但我不愿意这样，一边工作一边痛苦，这不值得。

我喜欢做出改变并接受挑战去做重要的事情和工作。钱固然很好，但我喜欢做出改变。

每个人最终都会到达一个工作、雇主、行业、甚至整个职业生涯结束的时刻。诚实并做出明智的决定比发现自己落后并可能被迫退出要好得多。

## 言论

1、

我们创造了一个文明，其中最重要的元素都深深地依赖于科学技术，但是我们还让科学技术变得如此难懂。这会带来灾难，我们可能暂时侥幸逃脱，但迟早，这种无知和权力的易燃混合物会爆炸。

-- [卡尔·萨根](https://www.openculture.com/2025/02/carl-sagan-predicts-the-decline-of-america-unable-to-know-whats-true.html)

1、

巴黎以前总是堵车，市长想出一个办法，大幅减少停车位，后来果然少有人开车了。

-- [CNN](https://edition.cnn.com/2026/03/21/travel/paris-transformation-anne-hidalgo-mayor)

1、

石油行业有一句谚语：一家健康的石油公司由一位地质学家负责，一家成熟的石油公司由一位工程师负责，一家衰落的石油公司由一位会计师负责，一家垂死的石油公司由一位律师负责。

-- [Hacker News 读者](https://news.ycombinator.com/item?id=45429423)

1、

如果你观察过程序员，可能会发现他们发呆的时间远比敲击键盘的时间多。不，他们（可能）并没有偷懒，程序员大部分工作都是在脑子里完成的。

-- [《AI 编码陷阱》](https://chrisloy.dev/post/2025/09/28/the-ai-coding-trap)


1、

通过“英语编程”来操作 AI，一遍遍编辑提示，试图获得最佳结果，这是一种不精确、缓慢且极其痛苦的做事方式。

-- [《手写该死的代码》](https://antonz.org/write-code/)

1、

人类是追求舒适的生物，我们总是选择阻力最小的道路，不这样做非常困难。但是，这样不利于一个人的成长，成长需要逆流而上，你需要刻意地为自己引入阻力。

-- [《阻力是成长的必要条件》](https://jameelur.com/blog/overcoming-friction-leads-to-growth)
