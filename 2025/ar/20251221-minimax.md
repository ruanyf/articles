...
我每次看 AI 产出的代码，总觉得那是别人的代码，不是我的。如果整个项目都用 AI 生成，潜意识里，我感觉不到那是自己的项目。我的习惯是，更愿意自己写新项目的主体代码。

**我主要把 AI 用在别人的项目和历史遗留代码**，这可以避免读懂他人代码的巨大成本。
...
我使用的 AI 模型，目前大多数时候是国产的 [MiniMax M2](https://www.minimaxi.com/)。我[测过](https://www.ruanyifeng.com/blog/2025/11/minimax-m2.html)它的功能，相当不错，能够满足需要，它的排名也很靠前。

![](https://cdn.beekka.com/blogimg/asset/202512/bg2025122105.webp)

而且，它有包月价（29元人民币），属于最便宜的编程模型之一，可以放心大量使用，反复试错。要是改用大家都趋之若鹜的 Claude 系列模型，20美元的 Pro 套餐不太够用，200美元的 Max 套餐又太贵。
...
因为跟自己相关，我挺关注这件事，就仔细看了发布说明，本次升级主要有四处增强。
...
处理历史遗留代码的第二步，是调整代码，修改功能，让老项目能够满足现在的需要。

我演示一个简单的修改，把链接颜色从默认的红色（下图）改成蓝色。原始代码中，颜色是写死的，根本没有留出修改的地方，而且分散在多处。

![](https://cdn.beekka.com/blogimg/asset/202512/bg2025122217.webp)

> 【提示词】将默认主题的 a 元素颜色，从红色（#ff3502）改成蓝色（#0000FF）
...
这时，AI 不仅会修改代码，还会有详细的说明，向你解释具体做了什么。这一点真的好，方便理解和将来追溯，我自己手写代码一般都懒得写这么详细的说明，这就是 AI 比人强的地方。

页面链接现在就从红色全部变成了蓝色（下图）。

![](https://cdn.beekka.com/blogimg/asset/202512/bg2025122216.webp)

## 八、

第三步就是为老代码添加一些原来没有的功能。

上一节说过，这个项目区分链接。如果是公众号内部链接，可以直接点击，否则就不能点击，而是把 URL 加入文末列表（下图）。

![](https://cdn.beekka.com/blogimg/asset/202512/bg2025122213.webp)

我打算增加一段逻辑，让内部链接的 URL 也加入文末列表。

> 【提示词】将 markdown 代码里面的微信公众号的链接，也加入页尾的链接列表
...
现在，公众号内部链接（即 mp.weixin.qq.com 域名下的链接），就同样加到了文末的链接列表（下图）。
...
演示就到此为止，历史遗留代码的解释、修改、功能增加，这三种情况都包括了。这就是我目前的 AI 编程的基本步骤了。

为了便于演示，我选了一个简单的示例，但是如果人工来做上面这些事情，从头到尾可能需要一个工作日，但是 AI 只需要十几分钟。

所以，以后遇到历史遗留代码，我一律建议先交给 AI。

至于这次体验的 MiniMax M2.1，我感觉不错。大家都看到了，我的提示词很简单，就是一句话，但是它正确理解了意图，基本一次就修改正确了。

而且，它就像发布说明说的一样，运行速度很快，思考过程和生成过程最多也就两三分钟，不像有的模型要等很久。

更令人满意的是，不管什么操作，它都会给出详细的讲解和代码注释。

总之，就我测试的情况来看，这个模型的 Web 开发能力很不错，可以用于实际开发。

我还想指出，虽然这次的示例比较简单，不能完全测出它的能力，但是我认为这不太重要。就像著名开发者 Simon Willison [最近说的](https://simonwillison.net/2025/Nov/24/claude-opus/)，评测大模型越来越困难，“我识别不出两个模型之间的实质性差异”，因为主流的新模型都已经足够强大，足以解决常见任务，只有不断升级评测的难度，才能测出它们的强弱。
