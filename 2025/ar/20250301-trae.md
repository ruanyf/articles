...
年初一月份，我就看到新闻，字节发布了国内第一款 AI IDE，叫做 [Trae](https://sourl.cn/g3vtkm)。
...
上周，我遇到字节的同学，得知 Trae 国内版就在本周发布，我还提前拿到了内测版。

今天，就来说说我的使用心得，顺便也探讨一个更大的问题：**AI 工具已经这么多了，该不该造新的轮子？**

先透露一下结论：Trae 国内版值得用。它作为一个 IDE，整体比插件好用，而且内置的 DeepSeek 满血版是无限量免费使用。

## 一、国外版与国内版

介绍使用体会之前，我先解释一下，两个版本的区别。

Trae 国外版使用国外的模型，界面全英文。对于国内用户来说，存在连接不上、等待时间长、网速较慢等问题。

Trae 国内版使用国内的模型，界面全中文，根据国内用户的习惯进行了定制，连接也比较稳定快速。
...
但是我必须说，Builder 模式目前还不完善，离理想状态有较大的差距。我试了多次，如果需求复杂一点，生成的代码很难一次就跑起来，需要多次较大的修改。这个模式后续还需要不断完善。
...
一旦指定了上下文，AI 的回答会更有针对性。比如生成代码时，就会介绍上下文的场景。
...
我听字节的同学说，Trae 后面会提供“多模态输入”，上传设计稿或设计稿，就能生成页面，希望能够早日开通。
...
（3）AI 模型（包括满血版 DeepSeek）无限量免费使用。
