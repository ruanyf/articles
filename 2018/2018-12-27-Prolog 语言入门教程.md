...
上面代码中，`X`是`Y`的母亲（`mother(X, Y) `）取决于两个条件。第一个条件是`Y`是`X`的小孩，第二个条件是`X`必须是女性。只有这两个条件都为`true`，才能得到`mother(X, Y)`为`true`。
...
（完）
<!--stackedit_data:
eyJoaXN0b3J5IjpbMjExMDQ2NzE3MiwtNDg1NDUzMzIwLDE2NT
