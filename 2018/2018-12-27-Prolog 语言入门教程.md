...
规则是推理方法，即如何从前提得到某个结论。

举例来说，我们定下一条规则：所有朋友关系都是相互的，即 jack 的朋友是 peter，就意味着 peter 也认为 jack 是朋友。规则写成下面这样。

```bash
friend(X, Y) :- friend(Y,X).
```

上面代码中，`X`和`Y`都是大写，表示这是两个变量。符号`:-`表示推理关系，可以读成“意味着”，`X`是`Y`的朋友，就意味着`Y`是`X`的朋友。
<!--stackedit_data:
eyJoaXN0b3J5IjpbLTEwMTg5NDAwMjQsLTk3NDYxNzgwOSwtNj
E3OTcxNTQyLDE1NDk0MzY1MjIsMjEwNDQxOTc5MywxNTI5NzMx
OTQ4LC02MzcxMTAsLTcwNDU1MTU3MF19
-->