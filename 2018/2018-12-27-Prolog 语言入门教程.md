...
上面代码中，`X`和`Y`都是大写，表示这是两个变量。符号`:-`表示推理关系，含义是只要右边的表达式`friend(Y,X)`为`true`，那么左边的表达式`friend(X, Y)`也为`true`。

可以读成“取决于”，`X`是`Y`的朋友，取决于`Y`是`X`的朋友。也就是说，只有`friend(Y,X)`为`true`，才能得到`friend(X, Y)`为`true`。
...
（完）
<!--stackedit_data:
eyJoaXN0b3J5IjpbMTk4MTc0ODA4NSwtNDg1NDUzMzIwLDE2NT
Q2NDQwNjIsLTU3MzY2NzkwOSwtOTIxNDI3OTM0LC0zOTk4MDA4
OTUsMTcwOTkwOTAwMCw1MjU0MzcyNDIsLTg0NzUxMTY1LDkyND
YwNTk3OCwtNzg2NDg5MDAyLC0xNDYwMDM3NTgxLC0xMzA0OTg0
OSw3NzE2Nzg3MiwtOTc0NjE3ODA5LC02MTc5NzE1NDIsMTU0OT
QzNjUyMiwyMTA0NDE5NzkzLDE1Mjk3MzE5NDgsLTYzNzExMF19
