...
> - [在线会议软件 Webex](https://business-review.eu/tech/cisco-webex-breaks-through-language-barriers-and-launches-real-time-translation-217869) 发布了实时会议翻译功能，支持100多种语言，让不同语言的人坐在一起、不通过翻译开会成为现实。

![](https://cdn.beekka.com/blogimg/asset/202103/bg2021031107.jpg)

> - [欧盟和英国](https://www.bbc.com/news/business-56340077)即将出台《维修权法》，要求厂商必须保证消费者有权对产品进行维修。具体来说，10年内厂商都必须提供产品零件，而且要让产品更容易修理。这样做的目的是，延长产品寿命，减少电子垃圾。
...
3、[浏览器是如何校验证书的？](https://cjting.me/2021/03/02/how-to-validate-tls-certificate/)（中文）

本文介绍了浏览器怎么处理 HTTPS 的证书，它的校验机制是什么，内容较为通俗全面。（[@cj1128](https://github.com/ruanyf/weekly/issues/1672) 投稿）

4、[使用 FFmpeg 将短视频转为 GIF 和 WebP 动画图片](https://mattj.io/posts/2021-02-27-create-animated-gif-and-webp-from-videos-using-ffmpeg/)（英文）

在某些场合，动画图片的兼容性要好于视频。本文是一个教程，介绍如何使用 FFmpeg 将视频转成动画图片。

5、[为什么 Java 后端程序员不愿意改用 Kotlin？](https://medium.com/google-developer-experts/why-are-java-server-side-developers-not-adopting-kotlin-8eb53e06ee99)（英文）

作者所在的团队从 Java 跳船到了 Kotlin。但是，大部分 Java 后端团队不愿意这样做，作者讨论了原因。

6、[ECMAScript 2021 的新语法](https://2ality.com/2020/09/ecmascript-2021.html)（英文）
...
在一个 SVG 文件里面，实现俄罗斯方块小游戏。这里的关键就是，SVG 可以内嵌`<script>`标签。
...
一个使用 EXIF 信息对图片文件批量改名的工具，支持 MacOS 和 Windows。（[@Arman19941113](https://github.com/ruanyf/weekly/issues/1674) 投稿）
...
一个 Chrome 浏览器插件，只有网页上有 GitHub 仓库链接，就在后面插入该仓库的星星数量。（[@chinazhangchao](https://github.com/ruanyf/weekly/issues/1675) 投稿）

9、[Issues Translate Action](https://github.com/tomsun28/issues-translate-action)

一个 GitHub Actions 脚本，定时运行生成一个 GitHub Issue 机器人，将非英语的 Issue 自动翻译成英语。（[@tomsun28](https://github.com/ruanyf/weekly/issues/1678) 投稿）
...
有些计算机语言，运算可能以返回空值（null），这是一个设计错误，因为会中断类型系统，你将无法依靠编译器来检查代码的有效性。
