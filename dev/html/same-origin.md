...
其中，最重要的限制就是”同源政策“（[same-origin policy](https://en.wikipedia.org/wiki/Same-origin_policy)），这是浏览器安全的基石。很多开发者都知道这一点，但又了解得不全面。
...
1995年，同源政策由 Netscape 公司引入浏览器。此后，所有其他浏览器都部署了这种政策。
...
同源政策的目的，主要是为了保证用户信息的安全，防止恶意的网站窃取数据。

让我们设想这样一种情况，A网页是一个银行网站，用户登录以后，又去浏览其他网站。如果其他网站可以读取A网站的 Cookie，会发生什么？很显然，如果 Cookie 包含隐私（比如存款总额），这些信息就会泄漏。
...
虽然这些限制是必要的，但是有时很不方便，合理的用途也无法完成。下面详细介绍，如何合理规避上面三种限制。

## 二、Cookie

Cookie 是服务器写入浏览器的一小段信息，只有同源的网页才能共享。但是，如果一级域名相同，只是二级域名不同，浏览器允许通过设置`document.domain`共享 Cookie。
...
但是，LocalStorage 和 IndexDB 无法通过这种方法，规避同源政策，而要使用下文介绍的PostMessage API。
