...
`signal.h`提供了信号处理（即异常情况）的工具。所谓“信号”（signal），可以理解成系统与程序之间的短消息，主要用来表示运行时错误，或者发生了异常事件。
...
信号处理函数`handler`接受一个 int 类型的参数，表示信号类型。函数体内部可以根据这个整数，判断到底接受到了哪种信号，因为多个信号可以共用同一个处理函数。一旦处理函数执行完成，程序会从信号发生点恢复执行。但是，如果遇到 SIGABRT 信号，处理函数执行完成，系统会让程序中止。
...
上面示例中，先为信号`SIGINT`指定了新的处理函数，等到这个函数用过之后，再恢复原来的处理函数。
...
它接受一个信号值作为参数，表示发出该信号。它的返回值是一个整数，可以用来测试信号发出是否成功，0 表示成功，非 0 表示失败。
