...
如果强制标准输入从文件产生，而不是从键盘产生，可以在文件名前面加上小于号`<`，跟着程序后面。这叫做“输入重定向”（input redirection）。

```bash
$ demo < in.dat
```

如果强制标准输出（或标准错误）写入一个数据文件，而不是显示器，可以在文件名前加上大于号`>`，跟在程序后面。这叫做“输出重定向”（output redirection）。
...
上面示例中，`getc()`依次读取文件的每个字符，将其放入变量`ch`，直到读到文件结尾，返回 EOF，循环终止。
...
C 语言规定，文件函数到达文件结尾处，要返回一个特殊字符 EOF。程序可以通过这个字符判断，是否读取完了文件流。
...
由于`fscanf()`可以连续读取，直到读到文件尾，或者发生错误（读取失败、匹配失败），才会停止读取，所以`fscanf()`通常放在循环里面。
...
`fgets()`读取 STRLEN - 1 个字符之后，或者遇到换行符与文件结尾，就会停止读取，然后在已经读取的内容末尾添加一个空字符`\0`，使之成为一个字符串。注意，`fgets()`会连换行符一起读取，放进字符串。

读取成功时，`fgets()`的返回值是它的第一个参数，即指向保存的字符串的指针，否则返回空指针 NULL。
