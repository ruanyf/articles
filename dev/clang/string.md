...
上面示例中，字符串`str2`复制给`str1`，但是复制长度最多为`str1`的长度减去1，`str1`剩下的最后一位用于写入字符串的结尾标志`\0`。这是因为`strncpy()`不会自己添加`\0`，如果截取的字符串片段不包含结尾标志，就需要手动添加。
...
`strncat()`的用法与`strncat()`完全一致，只是增加了第三个参数，指定最大添加的字符数。在添加过程中，一旦达到指定的字符数，或者在添加的字符串中遇到空字符`\0`，就不再添加了。它的原型定义在`string.h`头文件里面。
...
`strcmp()`函数用于比较两个字符串的内容。如果两个字符串相同，返回值为`0`，否则返回非零值。该函数的原型如下，定义在`string.h`头文件里面。
