...
注意，`\0`不是字符`0`（ASCII 码 48），也不是字符空白符（ASCII 码 32），而是二进制位都为`0`的一个标记，表示 NULL（空）。`\0`不用自己设置，C 语言会在生成字符串的时候自动添加。

C 语言约定，字符串必须放在双引号里面，以便跟字符类型（`char`）的值区分。字符类型的值只能放在单引号里面。
...
上面两种写法都声明了一个字符串变量`s`。如果采用第二种写法，由于字符数组的长度可以留给编译器自动计算，所以声明时可以不给出字符数组的长度。

```c
char s[] = "Hello, world!";
```

字符串数组的长度，可以大于字符串的实际长度。

```c
char s[50] = "hello";
```

上面示例中，字符串数组`s`的长度是`50`，但是字符串“hello”的实际长度只有6（包含结尾符号`\0`），所以后面空出来的44个位置，都会被初始化为`\0`。

字符串数组的长度，不能小于字符串的实际长度，会发生难以预料的结果。

```c
char s[5] = "hello";
```

上面示例中，字符串数组`s`的长度是`5`，小于字符串“hello”的实际长度6，这会导致最后的结尾符号`\0`，没有写入字符串数组。这很可能导致后面的代码报错。

字符指针和字符数组，这两种声明字符串变量的写法基本是等价的，但是有两个细微的差异。
...
上面示例中，字符数组不能指向另一个字符串。字符数组的数组名，总是指向初始化时的字符串地址，不能修改。
...
上面示例中，不能直接把字符串赋值给字符数组，会报错。原因是字符数组的变量名，跟所指向的数组是绑定的，不能指向另一个地址。

另外，需要注意的是，指针变量都需要有初始化的步骤，确定指向哪一个内存地址。未经初始化，不能修改它所指向的值。
...
上面示例中，字符指针`s`未经初始化，它指向一个随机的内存地址。这时，不能未经初始化，就去修改所指向的地址里面的值。

## strlen()

C 语言提供了一个`strlen()`函数，返回字符串的字符长度，不包括末尾的空字符`\0`。该函数的原型如下。
...
上面两种字符串复制写法，都是错的。因为数组的变量名是一个固定的地址，不能修改，指向另一个地址。
...
C 语言提供了`strcpy()`函数，用于将一个字符串的内容，复制到另一个字符串，相当于字符串赋值。该函数的原型定义在`string.h`头文件里面。

```c
strcpy(char dest[], const char source[])
```

`strcpy()`接受两个参数，第一个参数是目的字符串数组，第二个参数是源字符串数组。复制字符串之前，必须要保证第一个参数的长度不小于第二个参数，否则不会报错，但会溢出第一个字符串变量的边界，发生难以预料的结果。第二个参数的`const`说明符，表示不会修改第二个字符串。
...
`strcpy()`并不检查目标字符串能否容纳源字符串的副本，这会产生安全风险，所以 C 语言又提供了`strncpy()`函数，可以解决这个问题。`strncpy()`多了第3个参数，用来指定复制的最大字符数，防止溢出字符串变量的边界。
...
上面的代码是将字符串`str2`复制给`str1`，但是复制长度最多为`str1`的长度减去1，`str1`剩下的最后一位用于写入字符串的结尾标志`\0`。这是因为`strncpy()`不会自己添加`\0`，如果截取的字符串片段不包含结尾标志，就需要手动添加。
...
注意，`strcat()`的第一个参数的长度，必须足以容纳添加第二个参数字符串。否则，拼接后的字符串会溢出第一个参数的边界，影响到相邻的内存单元，这是很危险的。

## strncat()

由于`strcat()`需要使用者自己保证，不会产生变量溢出问题，所以 C 语言又提供了`strncat()`函数解决这个问题。

`strncat()`增加了第三个参数，指定最大添加的字符数。一旦发现，拼接的第二个字符串到达这个数量，或者遇到空字符`\0`，就不再拼接了。它的原型定义在`string.h`头文件里面。
...
`sprintf()`有严重的安全风险，如果字符串过长，超过了字符串指针变量保留的长度，`sprintf()`依然会将其写入，导致发生溢出。为了控制写入字符串的长度，C 语言又引入了另一个函数`snprintf()`。
...
`sprintf()`的返回值是写入变量的字符数量（不计入尾部的空字符`\0`），应该小于`n`。如果遇到错误，返回负值。
...
包含多个字符串的数组，需要通过二维的字符数组实现。每个字符串本身是一个字符数组，多个字符串再组成一个数组。
