...
上面示例里面的`const`，表示变量`PI`的值不会改变。如果改变的话，编译器会报错。
...
它的一个用途，就是防止在函数体内修改参数数组。有些函数接受数组作为参数，如果该数组在函数体内不会被改掉，就可以在函数声明时，对参数数组添加`const`说明符。这样的话，使用这个函数的人看到原型里面的`const`，就知道调用函数前后，参数数组保持不变。
...
上面示例中，`j`是一个指针变量，指向变量`i`。它本身没有`const`说明符，但是`i`有。这种情况下，`j`指向的值也不能被修改。
...
`static`对于全局变量和局部变量有不同含义。

（1）用于局部变量（位于块作用域内部）。

`static`表示在函数调用时，保持该变量的值，不会随着每次执行而初始化，就类似于一个只用于函数内部的全局变量。由于不必每次执行函数时，都对变量进行初始化，这样可以提高函数的执行速度。

（2）用于全局变量（位于块作用域外部）。

`static`表示该变量只用于当前文件，其他源码文件不可以引用该变量，即该变量不会被链接（link）。
...
只在当前文件里面使用的函数，也可以声明为`static`。其他文件就可以重复使用这些函数的名字。
...
`auto`说明符表示这个变量的存储，由编译器自主分配内存空间，只存在于定义时所在的作用域，退出作用域时会自动释放。

由于只要不是`extern`的变量（外部变量），都是由编译器自主分配内存空间的，这属于变量的默认行为。所以，该说明符没有实际作用，一般情况下，都省略不写。
...
上面代码中，`a`是`extern`变量，当前文件不会为它分配存储空间。
...
在函数内部使用`extern`声明变量，就相当于该变量是静态存储，每次执行时都要从外部获取它的值。

函数本身默认是`extern`，含义是可以被外部文件共享，通常是省略`extern`不写的。如果只希望函数在当前文件可用，那就需要在函数前面加上`static`。
...
`register`说明符向编译器表示，该变量是经常使用的，应该提供最快的读取速度，所以应该放进寄存器。但是，编译器可以忽略这个说明符，不一定按照它的指示行事。
