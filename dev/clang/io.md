...
注意，变量前面必须加上`&`运算符，因为`scanf()`传递的不是值，而是地址，即将变量`i`的地址改为用户输入的值的地址。
...
占位符`%s`其实不能简单地等同于字符串。它的规则是，从当前第一个非空白字符开始读起，直到遇到空白字符为止。由于`%s`遇到空格就会停止读取，所以无法用来读取多个单词，除非多个`%s`一起使用。这也意味着，`scanf()`不适合读取可能包含空格的字符串，比如书名或歌曲名。

`scanf()`占位符也可以像`printf()`那样，限定读取的长度。比如，`scanf("%10s", name)`会读取10个字符或者读到第一个空白字符为止。
...
上面示例中，`getchar()`每读取一个字符，长度变量`len`就会加1，直到读取到换行符为止。
