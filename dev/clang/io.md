...
写入数据时，程序把写入的数据放入缓存，等到缓存满了，缓存会一次性把里面的数据写入磁盘文件，程序再把后面的数据放入缓存，重复整个过程。

读取数据时，磁盘文件先把一部分数据放到缓存里面，然后程序从缓存获取数据，等到缓存空了，磁盘文件再把后面的数据放入缓存，重复整个过程。

这样的设计大大提高了程序的读写效率，因为缓存是在内存里面，读写的速度很快。当然，缓存跟磁盘的通信，还是比较慢的，但是一次性大块移动数据，还是要比每次一点点地读写数据快得多。

另一方面，对于程序来说，跟缓存通信有点像水流（stream），不是一次性读取或写入所有数据，而是一个持续不断的过程。先操作一部分数据，等到缓存处理完这部分数据，再操作下一部分数据。这个过程就叫做字节流操作。

C 语言的输入输出函数都属于字节流操作函数，输入函数操作的是输入流，输出函数操作的是输出流。
...
`scanf()`函数用于读取用户的键盘输入。程序运行到这个语句时，会停下来，等待用户从键盘输入。用户输入数据后，`scanf()`就会处理用户的输入，将其存入变量。
