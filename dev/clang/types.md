...
C 语言的数据是有类型（type）的。所谓”类型”，就是告诉编译器如何解释二进制的数据。

基本数据类型有三种：字符（char）、整数（int）和浮点数（float）。其他复杂的类型都是基于它们构建的。

## 字符类型

字符类型指的是单个字符，使用`char`表示。
...
在计算机内部，整数类型使用一个字节（8个二进制位）储存（`sizeof(char) == 1`）。C 语言底层将其视为整数，即`0`到`255`，字符根据 ASCII 码转为对应的整数，比如`B`对应整数`66`。
...
上面示例中，变量`t`等于单引号字符，由于字符的定义必须放在单引号里面，所以内部的单引号必须使用反斜杠转义。
...
一般来说，整数类型使用8个字节（64个二进制位）储存，可以表示的数据范围为`-9,223,372,036,854,775,808`到`9,223,372,036,854,775,807`。

整数类型默认是带符号的（signed），即带有正负号，既包括正数，也包括负数。`signed`这个类型修饰词默认可以省略。
...
一般来说，8个字节表示的整数太大了，真正用到的都是一些较小的整数值。C 语言提供三个修饰词，用来更精细表示整数类型的范围：`short`、`long`、`long long`。
