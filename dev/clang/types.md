...
有时候，某些字面量属于`int`，但是希望指定为`long`，这时可以为该字面量加上后缀`l`或`L`，编译器就知道要把这个字面量指定为`long`。建议后缀使用`L`，因为小写的`l`容易跟数字`1`混淆。
...
后缀`l`和`L`也可以用于八进制和十六进制，比如`020L`和`0x20L`。

小数（比如`3.14`）自动转为`double`类型。如果想指定为其他类型，需要在小数后面添加后缀`f`（float）或`lf`（long double）。
...
- `u`和`U`：表示`unsigned`，比如`15U`、`0377U`。`u`还可以与其他后缀结合，放在前面或后面都可以，比如`10ULL`和`10LLU`都是合法的。
