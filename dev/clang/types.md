...
- 表达式中的值是不同的整数类型时（比如`short`和`int`的混合计算），宽度较小的类型会提升为宽度较大的类型。有时还会将带符号的类型`signed`转为无符号`unsigned`。
...
上面示例中，变量`a`是带符号整数，`sizeof(int)`是`size_t`类型，这是一个无符号整数。由于`size_t`字节宽度更大，所以`a`会自动转成无符号整数，导致比较失败，`do_something()`不会执行。
