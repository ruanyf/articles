...
`default`分支用来处理前面的 case 都不匹配的情况，必须放在所有 case 的后面，因为它后面的 case 都不会执行到。
...
上面代码中，如果表达式`expression`为非零值（表示`true`），就会执行`statement`语句，然后再次判断`expression`是否为零；如果`expression`为零（表示`false`）就跳出循环，不再执行循环体。
