...
C 语言代码由一个个语句（statement）组成，分号是语句终止符，大多数语句必须以分号结尾。
...
多个语句可以写在一行，一个语句也可以写成多行，这时就要依靠分号判断语句是否结束，不能省略。
...
C 语言允许多个语句使用大括号`{}`，组成一个块，也称为复合语句（compounded statement），即在语法上将其视为多个语句组成的一个复合语句。
...
- `%lld`：十进制 long long int 类型。`ll`可以当作前缀，表示`long long`类型。
