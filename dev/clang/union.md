...
C 语言提供了 Union 结构，用来自定义可以灵活变更的数据结构。它内部可以包含各种属性，但同一时间只能有一个属性。这样做的最大好处是节省空间。
...
一旦为其他属性赋值后，原先可以取到值的`q.count`属性就不再有效了。因为所有属性都存储在同一段内存空间，后面写入的值会覆盖以前的值。除了这一点，Union 结构的其他用法与 Struct，基本上是一致的。
