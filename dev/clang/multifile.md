...
一个软件项目往往包含多个源码文件，编译时需要将这些文件编译成一个可执行文件。
...
上面命令中，gcc 的`-o`参数用来指定，生成的二进制可执行文件的文件名，本例是`foo`。
...
你可能马上就会想到，如果有多个文件都使用这个函数`add()`，那么每个文件都需要加入函数原型。一旦需要修改（比如改变函数参数的数量），就会非常麻烦。所以，通常的做法是新建一个专门的头文件`bar.h`，放置所有在`bar.c`里面定义的函数的原型。
...
然后使用`include`命令，在`foo.c`里面加载这个头文件`bar.h`即可。
...
最好避免这种重复加载，通常的做法是设置一个宏。一旦发现这个宏存在，就不再继续加载当前文件了。
...
上面示例中，`extern`说明符告诉编译器，变量`myvar`是其他脚本文件声明的，不需要在这里为它分配空间。

由于不需要分配空间，所以`extern`声明数组时，不需要给出数组长度。
...
有了 makefile，编译时，只要在 make 命令后面指定编译目标（编译产物的名字）即可。
