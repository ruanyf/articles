...
上面示例中，数组`a`包含三个整数成员，假定整数类型的字节长度是`4`，那么`sizeof(a)`返回的整个数组的字节长度就是`12`。
...
由于数组的起始地址是常用操作，`&array[0]`的写法有点麻烦，C 语言提供了便利写法，数组名等同于起始地址，也就是说，数组名就是指向第一个成员的指针。
