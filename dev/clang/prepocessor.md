...
注意，模式如果出现在字符串里面（即出现在双引号中），并不会被替换。
...
上面示例中，双引号里面的`TWO`并不会被替换。

## 宏

`#define`后面一共有两个参数。第一个参数就是要被替换的部分，称为`宏`（marco）。宏的名称不允许有空格，而且必须遵守 C 语言的变量命名规则，只能使用字母、数字与下划线（`_`），而且首字符不能是数字。第二个参数是替换体。
