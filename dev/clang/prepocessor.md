...
预处理器首先会清理代码，比如删除注释、多行的语句合成一个逻辑行等等。然后执行`#`开头的预处理指令。下面介绍 C 语言的预处理指令。

预处理指令可以出现在程序的任何地方，但是习惯上，往往放在代码的开头部分。

每个指令都以`#`开头，放在一行的行首，指令前面可以有空白字符（比如空格或制表符）。`#`和指令的其余部分之间也可以有空格，但是为了兼容老的编译器，一般不留空格。

所有指令都是一行的，除非在行尾使用反斜杠，将其折行。指令结尾处不需要分号。
...
一般来说，应该首先使用函数，它的功能更强、更容易理解。宏有时候会产生意想不到的替换结果，而且往往只能写成一行，除非对换行符进行转义，但是可读性就很差。

宏的优点是相对简单，本质上是字符串替换，不涉及数据类型，不像函数必须定义数据类型。而且，宏省掉了函数调用的开销，所以性能会好一些。另外，以前的代码大量使用宏，尤其是简单的数学运算，需要对它有所了解。
...
上面示例中，`STR(3.14159)`会被替换成`3.14159`。如果`x`前面没有`#`，这原本会被解释成一个浮点数，有了`#`以后，就会被转换成字符串。
...
上面示例中，替换后的文本`i1`、`i2`、`i3`是三个标识符，参数`n`是标识符的一部分。从这个例子可以看到，`##`运算符主要用来批量生成标识符。
...
有时候想新定义一个宏，但不确定是否以前定义过，就可以先用`#undef`取消，然后再定义。`#undef`的参数如果是不存在的宏，不会报错。
...
GCC 编译器的`-I`参数，也可以用来指定加载路径。

```bash
$ gcc -Iinclude/ -o code code.c
```

上面命令中，`-Iinclude/`指定从当前目录的`include`子目录里面，加载用户自己的头文件。

`#include`最常见的用途，就是用来加载包含函数原型的头文件（后缀名为`.h`），参见《多文件编译》一章。多个`#include`指令的顺序无关紧要，多次包含同一个头文件也是合法的。

## #if...#endif

`#if...#endif`指令用于条件判断，满足条件时，内部的行会被编译，否则就被编译器忽略。
...
上面示例中，`#if`后面的`0`，表示判断条件不成立。所以，内部的变量定义语句会被编译器忽略。`#if 0`这种写法常用来当作注释使用。

`#if`后面的判断条件，通常是一个表达式。如果表达式的值不等于`0`，就表示判断条件为真，则编译内部的语句；如果表达式的值等于0，表示判断条件为伪，则忽略内部的语句。
...
注意，没有定义过的宏，等同于`0`。因此`#if UNDEFINED`为伪，而`#if !UNDEFINED`为真。

`#if`的常见应用，就是打开（或关闭）调试模式。
...
GCC 的`-D`参数可以编译时指定宏的值，因此可以很方便地打开调试开关。
...
上面示例中，通过`#elif`指定了第二重判断。注意，`#elif`的位置必须在`#else`之前。如果多个判断条件皆不满足，则执行`#elase`的部分。
...
- `__STDC_VERSION__`：编译使用 C 版本，为一个格式为`yyyymmL`的长整数，C99 为“199901L”，C11 为“201112L”，C17 为“201710L”。
