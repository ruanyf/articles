...
注意，Enum 内部的常量名，可以不是大写，但是通常都使用大写。
...
Enum 的作用域与变量相同。如果是在顶层声明，那么在整个文件内都有效；如果是在代码块内部声明，则只对该代码块有效。这一点与`#define`声明的宏不一样，后者从声明处到文件结束都有效。如果与使用`int`声明的常量相比，Enum 声明的好处是更清晰地表示代码意图。
...
上面示例中，Enum 类型的名称是`enum resource`，而不是`resource`。`r`是`enum resource`的实例，编译器将其视为一个整数，也就是说，它应该等于`enum resource`里面声明的一个常量。
