...
联合类型（union types）指的是多个类型组成的一个新类型，某个值只要属于其中一个基础类型，就同时属于这个新类型。

联合类型使用符号`|`表示。举例来说，类型`A`与类型`B`组成的联合类型，用`A|B`表示。任何一个值只要属于类型`A`或类型`B`，都同时属于联合类型`A|B`。
...
交叉类型（intersection types）指的多个类型组成的一个新类型，这个新类型同时满足所有基础类型的特征。

交叉类型使用符号`&`表示。举例来说，类型`A`与类型`B`组成一个交叉类型，用`A&B`表示，它同时满足`A`和`B`的特征。
...
上面示例中，类型`B`是一个交叉类型，就在`A`的基础上增加了属性`bar`。
