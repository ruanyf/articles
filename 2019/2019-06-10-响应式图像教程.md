...
像素密度的适配，只适合显示区域一样大小的图像。如果希望不同尺寸的屏幕，加载不同大小的图像，`srcset`属性就不够用了，必须搭配`sizes`属性。
...
上面两节分别解决了像素密度和屏幕尺寸的适配，但是如果要在不同尺寸的屏幕，分别适配不同的像素密度，应该怎么办呢？
...
上面代码中，`<source>`标签的`media`属性给出屏幕尺寸的适配条件，每个条件都用`srcset`属性再给出两种像素密度的图像 URL。
