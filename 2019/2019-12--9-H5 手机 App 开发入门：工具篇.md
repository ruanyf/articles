...
混合技术栈使用 HTML 语言编写页面，再用 WebView 控件加载页面，所以只写一次页面，就能支持多个平台。跨平台技术栈也能做到跨平台支持，但是原理完全不同。

这类框架定义自己的语法编写页面，编译的时候再将其转为原生控件，生成原生 App。这样就完全解决了 Web 页面性能不佳的问题。下面介绍三个这一类的框架。
