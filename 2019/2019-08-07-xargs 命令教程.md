...
但是，大多数命令都不接受标准输入作为参数，只能直接在命令行输入参数，这导致无法用管道命令传递数据。举例来说，`echo`命令就不接受管道传参。

```bash
$ echo "hello world" | echo
```

上面的代码不会有输出。因为管道右侧的`echo`不接受管道命令传来的标准输入。这时，就需要`xargs`命令了。
...
真正执行的命令，紧跟在`xargs`命令后面，接受`xargs`命令传来的参数。

常用命令`rm`、`mkdir`、`ls`都需要`xargs`，转换标准输入。
...
大多数情况下，`xargs`命令都是结合管道命令一起使用。但是，它也可以单独使用。
...
输入上面的命令以后，命令行就会等待用户输入，作为标准输入。你可以输入任意内容，然后按下`Ctrl d`，表示输入结束，这时`echo`命令就会把前面的输入打印出来。
...
默认情况下，`xargs`将换行符和空格作为分隔符，依次作为命令行参数。

```bash
$ echo "one two three" | xargs mkdir
```

上面代码中，`mkdir`会新建三个子目录，因为`xargs`将`one two three`转为三个命令行参数。
...
另外，还有一个原因，使得`xargs`命令特别适合`find`命令。有些命令（比如`rm`）一旦参数过多会报错“参数列表过长”，而无法执行，改用`xargs`就没有这个问题，因为它对每个参数执行一次命令。
...
上面命令指定了每一行（`-L 1`）作为命令行参数，运行一次命令（`find -name`）。
...
上面命令指定将一项（`-n 1`）作为命令行参数，运行一次命令（`find -name`）。
...
如果`xargs`要将命令行参数，传给多个命令，可以使用`-I`参数。
...
上面代码中，`foo.txt`是一个三行的文本文件。我们希望对每一项命令行参数，执行两个命令（`echo`和`mkdir`），使用`-I file`表示`file`是命令行参数的替代字符串，执行的使用具体的参数会替代掉`echo file; mkdir file`里面的两个`file`。

## 十、--max-procs 参数

`xargs`默认只用一个进程执行命令。如果命令要执行多次，必须等上一次执行完，才能执行下一次。

`--max-procs`参数指定同时用多少个进程并行执行命令。`--max-procs 2`表示同时最多可以新建两个进程，`--max-procs 0`表示不限制。

```bash
$ docker ps -q | xargs -n 1 --max-procs 0 docker kill
```

上面代码表示，同时关闭尽可能多的 Docker 容器，这样运行速度会快很多。
