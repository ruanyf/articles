# 腾讯混元大模型 + WorkBuddy 组合实测，对比 GLM 5.1 与 DeepSeep 4 Flash

## 一、引言

本周，腾讯发布了全新的混元大模型 Hy3。

![](https://cdn.beekka.com/blogimg/asset/202607/bg2026070502.webp)

这个模型跟它的前序版本很不一样。混元作为腾讯自研的旗舰模型，第一个版本是2023年9月发布的，第二个版本是2025年12月。现在，在首次亮相近三年后，终于迎来了第三个版本。

腾讯显然对它寄予厚望，4月份就公开了预览版 [Hy3-preview](https://www.tencent.com/zh-cn/articles/2202320.html)，供大家测试，明确表示编码能力和 Agent 能力有大幅提升。

而且，今年3月腾讯还推出了配套工具 [WorkBuddy](https://www.codebuddy.cn/work/)。

![](https://cdn.beekka.com/blogimg/asset/202607/bg2026070508.webp)

WorkBuddy 的官方定位是“桌面的 AI 原生工作台”，支持各种 AI 使用场景。但是，现在很清楚了，发布它的一个重要原因就是为了给 Hy3 配套。

为了推广这个组合，现在只要在 WorkBuddy 里面试用 Hy3，就有两周的免费期。

![](https://cdn.beekka.com/blogimg/asset/202607/bg2026070562.webp)

上面就是 WorkBuddy 内置的可供选择的模型。

为了验证 Hy3 + WorkBuddy 这个组合，到底是不是像宣传的那样好用，我就利用现在这段免费期，从里面挑选了 GLM 5.1 和 DeepSeek 4 Flash，与 Hy3 进行 PK。

为什么选择这两个 PK 选手，而不是更新更强的模型，我下面就解释。

## 二、模型定位

首先，我们要搞清楚，混元 Hy3 到底是一个怎样定位的模型。

它的评分，截止我撰文时还没有公布。我只能说，从参数来看，它属于中等规模的模型，而不是那种非常大的大型模型：它采用混合专家模型（MoE）架构，总参数 295B，运行时激活的参数 21B，上下文长度 256K。

跟大型模型相比，这些参数并不大。这表明，它内部的计算量肯定也不大，比不了那些运算规模极大的 SOTA 模型。

为了比较它的性能，就要选那些跟它规模相近的模型。所以，我选了两个类似规模的国产模型，与它进行对比：GLM 5.1 和 DeepSeek 4 Flash。

![](https://cdn.beekka.com/blogimg/asset/202607/bg2026070561.webp)

从参数看，GLM 5.1 的计算量应该最大，混元 Hy3 其次，最后是 DeepSeek 4 Flash。

我的实际测试也是如此，DeepSeek 4 Flash 速度最快，混元 Hy3 其次，GLM 5.1 最慢。

由此可见，混元 Hy3 的设计思路不是追求最强的性能，而是追求性能和成本/运算速度的平衡，成为一个性能不错、成本较低、速度较快的可以日常经常使用的模型。

## 三、BridgeBench 测试基准

目前，AI 模型最热门的测试基准大概是 [BridgeBench](https://www.bridgebench.ai/)，号称“全世界排名第一的氛围编程测试基准”。

![](https://cdn.beekka.com/blogimg/asset/202607/bg2026070506.webp)

![](https://cdn.beekka.com/blogimg/asset/202607/bg2026070507.webp)

它一共170余道题，专门测试模型的代码能力。

它的运行方法和评分方法，都没有公开，但是[部分测试题](https://www.bridgebench.ai/test-prompts)是公开的。我选择了一些题目，上面三个模型都跑一下，看谁的表现最好。

不过，这些题目都很大很复杂，跑起来太费时间，有时一道题要跑一个小时。所以，前面两个简单测试，我就用[自己收集的测试题](https://github.com/ruanyf/ai-test-case)了。

另外，以下所有测试都是在 WorkBuddy 里面完成的。我只是切换模型和工作目录，其他条件都保持不变。

## 四、网页 UI 测试

我的第一个测试最简单，要求模型重构一个网站的 UI，使其成为美观有效的商务网站入口页（[提示词](https://github.com/ruanyf/ai-test-case#case01)）。

**混元 Hy3**

![](https://cdn.beekka.com/blogimg/asset/202607/bg2026070541.webp)

![](https://cdn.beekka.com/blogimg/asset/202607/bg2026070542.webp)

![](https://cdn.beekka.com/blogimg/asset/202607/bg2026070543.webp)

**GLM 5.1**

![](https://cdn.beekka.com/blogimg/asset/202607/bg2026070547.webp)

![](https://cdn.beekka.com/blogimg/asset/202607/bg2026070548.webp)

![](https://cdn.beekka.com/blogimg/asset/202607/bg2026070549.webp)

**DeepSeek 4 Flash**

![](https://cdn.beekka.com/blogimg/asset/202607/bg2026070544.webp)

![](https://cdn.beekka.com/blogimg/asset/202607/bg2026070545.webp)

![](https://cdn.beekka.com/blogimg/asset/202607/bg2026070546.webp)

显然，这三个模型的结果都很好，我觉得分不出差距。

网页 UI 应该算已经解决的问题了，已经不足以考验模型能力了，以后大概不用再测了。最多就是向大家展示一下，模型生成的网页样式和美感。

唯一可挑剔的，就是它们明显都用了 Tailwind 的模版或技能，导致看上去很雷同，没有个性。幸好生成的是商务网站，如果是个人网站，就太乏味了。

## 五、网页 2D 游戏《愤怒的小鸟》

第二个测试是生成网页版的《愤怒的小鸟》（[提示词](https://github.com/ruanyf/ai-test-case#case03)）。这个测试其实难度很大，尤其是小鸟的飞行和碰撞效果。

**混元 Hy3**

![](https://cdn.beekka.com/blogimg/asset/202607/bg2026070550.webp)

![](https://cdn.beekka.com/blogimg/asset/202607/bg2026070551.webp)

![](https://cdn.beekka.com/blogimg/asset/202607/bg2026070552.webp)

这是我测过的最佳结果之一，页面跟原始 UI 很接近，游戏完全可玩，小鸟的飞行和碰撞效果很好，小鸟落在地面上会反弹，砖块被碰撞后会消失。

**GLM 5.1**

![](https://cdn.beekka.com/blogimg/asset/202607/bg2026070553.webp)

![](https://cdn.beekka.com/blogimg/asset/202607/bg2026070554.webp)

![](https://cdn.beekka.com/blogimg/asset/202607/bg2026070555.webp)

这个结果也还可以，虽然 UI 跟原始游戏有差距，但是小鸟的飞行效果做得很好，尤其是玻璃碰撞后的粉碎效果很漂亮。

![](https://cdn.beekka.com/blogimg/asset/202607/bg2026070556.webp)

![](https://cdn.beekka.com/blogimg/asset/202607/bg2026070557.webp)

**DeepSeek 4 Flash**

![](https://cdn.beekka.com/blogimg/asset/202607/bg2026070558.webp)

![](https://cdn.beekka.com/blogimg/asset/202607/bg2026070559.webp)

这个结果也是可玩的，但是明显比其他两个模型逊色一些，无论是 UI 还是游戏的动画效果。

至于混元 Hy3 和 GLM 5.1，我个人感觉，前者更好一些。

## 六、2D 台球游戏

这个测试要求使用 Python 语言，生成一个 2D 的8球台球游戏（[提示词](https://www.bridgebench.ai/prompts/c4541e6f-be24-4b0b-ad5e-9ae5b8cfc1b7)）。

测试要求是只使用 `pygame-ce` 和 `numpy` 这两个软件包，前者负责处理游戏窗口，后者负责处理台球碰撞的数学计算。

**混元 Hy3**

![](https://cdn.beekka.com/blogimg/asset/202607/bg2026070515.webp)

![](https://cdn.beekka.com/blogimg/asset/202607/bg2026070516.webp)

![](https://cdn.beekka.com/blogimg/asset/202607/bg2026070518.webp)

这个结果出乎意料的好，画面精美，游戏完全是可玩的，碰撞效果逼真。

**GLM 5.1**

![](https://cdn.beekka.com/blogimg/asset/202607/bg2026070519.webp)

这个结果完全失败，台球就是黑色方块里面一个平面的圆，甚至方块的四角都没有去除。

![](https://cdn.beekka.com/blogimg/asset/202607/bg2026070520.webp)

而且，这个游戏不可玩，无论我怎么尝试，始终无法用球杆将白球击出。

**DeepSeek 4 Flash**

![](https://cdn.beekka.com/blogimg/asset/202607/bg2026070521.webp)

![](https://cdn.beekka.com/blogimg/asset/202607/bg2026070522.webp)

这个结果还可以，也是可玩的。但是，开球方式不对，是横过来开球，而且很难操作，需要鼠标来瞄准，然后用 Space 键击出。

显然，这一轮的赢家是混元 Hy3。

## 七、3D 鹈鹕骑自行车

这个测试要求使用 GLSL 着色器语言，生成 3D 的“鹈鹕骑自行车”场景（[提示词](https://www.bridgebench.ai/prompts/64269eb6-6691-4d05-9489-91fb7f4ddf08)）。

模型生成的将是一个 GLSL 脚本，把它贴到 [shardtoy.com](https://www.shadertoy.com/) 进行在线编译就能看到渲染结果。

**混元 Hy3**

![](https://cdn.beekka.com/blogimg/asset/202607/bg2026070513.webp)

![](https://cdn.beekka.com/blogimg/asset/202607/bg2026070514.webp)

**GLM 5.1**

![](https://cdn.beekka.com/blogimg/asset/202607/bg2026070511.webp)

![](https://cdn.beekka.com/blogimg/asset/202607/bg2026070512.webp)

**DeepSeek 4 Flash**

![](https://cdn.beekka.com/blogimg/asset/202607/bg2026070509.webp)

![](https://cdn.beekka.com/blogimg/asset/202607/bg2026070510.webp)

这一轮，三个模型的生成结果，都不算很理想。混元 Hy3 相对还好一点，但是自行车更像平衡车，由于提示词是英语 Bicycle，我不知道这个词是否还包括两轮的平衡车，否则就很难解释这种形状。

GLM 5.1 的自行车就更奇怪了，轮子成了水平的转盘，这是水上自行车吗？至于 DeepSeek 4 Flash，完全没有生成鹈鹕。

## 八、大型 3D 网页游戏

最后是最难的测试，要求模型生成大型的复杂网页游戏。

我先测了生成《MineCraft》，要求所有代码包含在单个的 HTML 文件（[提示词](https://www.bridgebench.ai/prompts/e3aca9dc-5d6d-4002-ab9c-76267f9689b2)）。

**混元 Hy3**

![](https://cdn.beekka.com/blogimg/asset/202607/bg2026070524.webp)

![](https://cdn.beekka.com/blogimg/asset/202607/bg2026070529.webp)

![](https://cdn.beekka.com/blogimg/asset/202607/bg2026070525.webp)

虽然画面比较简陋和残缺，但是游戏要素是完整的，你看得出来这是 MineCraft，一个开放的动态世界，可以无限漫游，使用 wasd 这四个键进行移动。

你甚至可以砍树收集材料。

![](https://cdn.beekka.com/blogimg/asset/202607/bg2026070526.webp)

相比之下，其他两个模型的生成结果，就有点惨不忍睹。

**GLM 5.1**

![](https://cdn.beekka.com/blogimg/asset/202607/bg2026070527.webp)

![](https://cdn.beekka.com/blogimg/asset/202607/bg2026070528.webp)

**DeepSeek 4 Flash**

![](https://cdn.beekka.com/blogimg/asset/202607/bg2026070530.webp)

![](https://cdn.beekka.com/blogimg/asset/202607/bg2026070531.webp)

虽然都很糟糕，GLM 5.1 生成的至少还是一个 3D 世界，可以四处移动，DeepSeek 4 Flash 生成的就是一个黑屏，什么也没有，也无法操作。

我怀疑是不是测试过程有问题，于是又测了生成 3D 开放世界《罪恶城市》（[提示词](https://www.bridgebench.ai/prompts/709d9a91-7406-4240-8b23-ef4a456c385c)）。

**混元 Hy3**

![](https://cdn.beekka.com/blogimg/asset/202607/bg2026070532.webp)

![](https://cdn.beekka.com/blogimg/asset/202607/bg2026070534.webp)

![](https://cdn.beekka.com/blogimg/asset/202607/bg2026070535.webp)

我觉得这个城市的视觉效果还不错，可以第一人称视角无限漫游，右上角还有实时地图。

另外，路上还有行人，你可以按键发射子弹，同时听到音效。

![](https://cdn.beekka.com/blogimg/asset/202607/bg2026070536.webp)

**GLM 5.1**

![](https://cdn.beekka.com/blogimg/asset/202607/bg2026070537.webp)

![](https://cdn.beekka.com/blogimg/asset/202607/bg2026070538.webp)

**DeepSeek 4 Flash**

![](https://cdn.beekka.com/blogimg/asset/202607/bg2026070539.webp)

![](https://cdn.beekka.com/blogimg/asset/202607/bg2026070540.webp)

GLM 5.1 直接黑屏，DeepSeek 4 Flash 能运行，但画面一点看不出是个城市。可以确认了，这两个模型在 3D 网页游戏方面的能力比较弱。

## 九、总结

经过这些测试，混元 Hy3 的表现让人眼前一亮，简单的任务不成问题，复杂的任务也能做出可以接受的结果。

它的表现明显好于 DeepSeek 4 Flash，很多时候也好于 GLM 5.1，至少不比它差。

考虑到它的运行速度不慢、成本相对低，确实是一个性价比很突出的模型，各方面比较均衡，完全可以作为日常主力的编程/Agent 模型使用。

我对 [WorkBuddy](https://www.codebuddy.cn/work/) 的评价也不错，尽管它的定位是全场景的 AI 原生工作台，但作为编程代理使用也完全没有问题，权限控制、代码高亮、结果预览等功能全部支持。

腾讯为了推广 WorkBuddy，支持力度非常大，目前在里面使用 Hy3 是免费期，感兴趣的朋友正好试试这个模型。

（完）

# Hy3 

Agent / Coding

Agent方向能力，验证模型在真实工程任务中的表现。

Hy3 正式版是一个快慢思考融合的 MoE 语言模型，总参数 295B、激活参数 21B，最大支持 256K 上下文长度，整体性能达到同尺寸模型最佳水平，特别适用于 Coding 与智能体（Agent）类场景。

Coding Agent 任务执行与生产力场景跃升以及复杂推理和数理能力的优化，

- Coding Agent 能力大幅升级（前端深度优化）：针对前端开发场景进行了深度优化，在前端需求理解、组件生成、交互逻辑编写及自动化 Debug 方面的可用性显著提升，旨在打造更流畅的开发闭环。
- Working Agent 通用办公能力全面拓宽：强化了面对复杂工作流时的任务规划、长文本信息处理与多工具调用能力，进一步贴近实际业务场景的自动化需求。
- Reasoning / STEM 复杂推理与数理能力显著跃升，突破 Agentic Reasoning瓶颈：充分释放了思考和工具使用的潜力，模型在多步骤逻辑推演、复杂数学计算及科学问题求解上的表现得到大幅强化。凭借更强的长链路规划、动态反思与使用外部工具进行数据分析、科学研究，模型能精准拆解并攻克复杂问题，为高阶数据分析、科研等硬核任务提供更可靠的智力。

---

上线后前2周全量在WorkBuddy限时免费开放

2023年9月 混元大模型发布
2025年12月，腾讯混元2.0模型发布
2026年4月23日，腾讯发布并开源混元Hy3 preview。

---

HY3：总参数 295B、激活参数 21B，最大支持 256K 上下文长度
GLM 5.1：200k token 上下文窗口，744B 总参数，有效计算量约 40B
DeepSeek V4 Flash：总参数量 284B、激活参数量 13B，拥有 100 万 token 的上下文窗口

