...
但那只是一个裸页面，今天接着往下讲，如何为这个页面添加样式，使它看上去更美观。
...
小程序允许在顶层放置一个`app.wxss`文件，里面采用 CSS 语法设置页面样式。这个文件对所有页面都有效。
...
实际开发中，直接对`<text>`标签设置样式，会影响到所有的文本。一般不这样用，而是通过`class`属性区分不同类型的文本，然后再对每种文本类型设置样式。
...
首先，在`pages/home`目录里面，新建一个`home.wxss`文件，这个文件设置的样式，只对 home 页面生效。每个页面通常有不一样的布局，所以页面布局一般不写在全局的`app.wxss`里面。
...
页面样式就讲到这里，下一篇教程讲解怎么在小程序里面加入 JavaScript 脚本，跟用户互动。
