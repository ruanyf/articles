...
> - [全球的 IPv4 地址](http://www.southgatearc.org/news/2020/october/sale-of-amateur-radio-amprnet-tcp-ip-addresses.htm)历史上有1600万个分配给了业余无线电协会。由于 IPv4 地址已经没有剩余了，最近亚马逊公司花了1.08亿美元，向该协会购买了400万个 IPv4 地址。
...
Unity 是世界最流行的游戏开放软件之一，本文是它的初学者教程，介绍一些游戏开放的最基本概念，不涉及代码。
...
现在有一些 CDN 专门供 JS 脚本使用，本文认为这种做法并不可取。最有说服力的一条理由大概是，由于 Chrome 86 开始禁止使用其他网站留下的缓存，脚本共享 CDN 的最大好处就消失了：不同网站的脚本使用同一个 CDN，并不会令加载变快。
...
作者讲解如何自己写一个 React 实现，支持 JSX 语法和一些基本 API，压缩后代码不超过 1KB。
...
10、[tiptap](https://github.com/ueberdosis/tiptap)
