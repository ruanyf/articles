...
6、[Chrome 浏览器改变缓存机制](https://www.cnbeta.com/articles/tech/1038795.htm)（中文）

用户访问网页以后，浏览器会缓存静态资源。这本来是为了以后再访问这个页面时，加快显示速度，但也可用来追踪用户，通过识别缓存，了解用户访问过哪个网站。

Chrome 86 开始有了新的缓存防追踪机制，具体来说，就是为缓存加上数字密钥，使得第三方脚本无法加载缓存。
