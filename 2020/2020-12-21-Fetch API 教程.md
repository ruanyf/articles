...
`fetch()`的第一个参数是 URL，还可以接受第二个参数，作为配置对象，定制发出的 HTTP 请求。HTTP 请求的方法、标头、数据体都在这个对象里面设置。
...
`credentials`属性指定是否发送 Cookie。它可能的取值如下：
...
`signal`属性指定一个 AbortSignal 实例的信号对象，用于取消`fetch()`请求，详见下一节。

**keepalive**

`keepalive`属性用于页面卸载时，告诉浏览器在后台保持连接，继续发送数据。

一个典型的场景就是，用户离开网页时，脚本向服务器提交一些用户行为的统计信息。这时，如果不用`keepalive`属性，数据可能无法发送，因为浏览器已经把页面直接卸载了。
...
`redirect`属性指定 HTTP 跳转的处理方法。它可能的值如下：

- `follow`：默认值，`fetch()`跟随 HTTP 跳转。
- `error`：如果发生跳转，`fetch()`就报错。
- `manual`：`fetch()`不跟随 HTTP 跳转，但是`response.url`属性会指向新的 URL，`response.redirected`会变为`true`，由开发者自己决定后续如何处理跳转。
...
`referrerPolicy`属性用于设定`Referer`标头的规则。它可能的值如下：
...
上面示例中，首先新建 AbortController 实例，然后发送`fetch()`请求，配置对象的`signal`属性必须指定接收 AbortController 实例发送的信号。
