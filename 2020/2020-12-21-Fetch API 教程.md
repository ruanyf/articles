...
初学者学完 JavaScript 的语法，开始进入实际的网页编程，一定有人告诉你，要掌握一个叫做 XMLHttpRequest 的东西。

脚本都靠它发出 HTTP 请求，跟服务器通信。所谓的 AJAX 操作就是基于它而实现的。要是没有它，就不会有单页应用。
...
就是因为写起来麻烦，实际开发中，使用的往往是它的各种封装库，比如前期的 jQuery。
...
但是，回调函数的写法不直观，后来的封装库都采用 Promise 的写法，比如 axios。
...
现在用起来方便了，但是每次使用都需要加载外部库，而它偏偏是一个那么基本的需求。所以后来，浏览器的开发者决定抛弃 XMLHttpRequest，另起炉灶，开发一套全新的 API。
...
我一直想写一篇教程，完整介绍它的各种用法，也便于自己使用的时候查找。现在终于写出来，以后可以告别 XMLHttpRequest 了。

这篇教程的内容比较长，超过一万字，代码也非常多，所以不贴在公号了。想要阅读的朋友，请点击“阅读原文”，或者访问下面的网址。
