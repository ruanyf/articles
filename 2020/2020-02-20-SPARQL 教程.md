...
这一节就演示，如何通过维基数据查询，山西省人口最多的地区。

首先，进入[维基数据网站](https://www.wikidata.org/)，在页面顶部的搜索栏，搜索“山西”。
...
关于 select 查询的更多语法例子，可以参考[这篇文章](https://www.cambridgesemantics.com/blog/semantic-university/learn-sparql/sparql-nuts-bolts/)。
...
```sql
SELECT ?programmer ?programmerLabel
WHERE {
...
```sql
SELECT ?programmer ?programmerLabel ?notableworkLabel
WHERE {
  ?programmer wdt:P106 wd:Q5482740 .
  ?programmer rdfs:label ?programmerLabel .  
  FILTER ( LANGMATCHES ( LANG ( ?programmerLabel ), "zh-CN" ) )
