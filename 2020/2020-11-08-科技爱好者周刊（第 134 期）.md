...
宝马公司推出了全世界第一套电动的飞行翼装，只要穿上它，就能自由在天空飞翔。传统的飞行翼装是无动力的，只能在天空滑翔，这种新的翼装依靠电力驱动。它的胸前有一对电动涡轮，能够提供5分钟的 15 kW 推力。
...
本周的一件大事是，苹果公司发布了 macOS 的新版本 Big Sur。

1、

一位设计师贴出了 Big Sur 与 上一个版本 Catalina 的 [UI 比较](https://www.andrewdenty.com/blog/2020/07/01/a-visual-comparison-of-macos-catalina-and-big-sur.html)。Big Sur 明显变得更平面化，减少了对比度和阴影，颜色也更活泼。
...
许多用户升级后发现，只要不是从应用商店下载的应用，就无法打开，会失去响应。

一位程序员指出，原因是苹果的  [OCSP 服务器](https://blog.cryptohack.org/macos-ocsp-disaster)超载了，因为升级的人太多，都向 OCSP 服务器发出请求。

![](https://www.wangbase.com/blogimg/asset/202011/bg2020111608.jpg)

3、

OCSP 服务器主要用来验证，应用的开发者证书是否有效。但这也意味着，用户每打开一个应用，苹果公司都会得到通知。这引发了[广泛的担忧](https://sneak.berlin/20201112/your-computer-isnt-yours/)，苹果掌握了太多的用户信息。

更糟的是，OCSP 请求没有加密，是明文的，第三方可以拿到这些数据。另外，苹果允许某些自家的应用（比如应用商店）[绕过防火墙和 VPN](https://thenextweb.com/plugged/2020/11/16/apple-apps-on-big-sur-bypass-firewalls-vpns-analysis-macos/)，与苹果服务器通信，用户隐私完全暴露了。

4、

苹果公司[最新回应](https://www.macrumors.com/2020/11/15/apple-privacy-macos-app-authenticaion/)：

- OCSP 服务器不收集用户数据，并且计划从日志中删除所有IP信息。
- 明年的系统升级将加密 OCSP 请求，并且应用不会因为这个原因失去响应，用户也可以选择关闭证书验证。
...
6、[如何使用工具写出可维护的 Node.js 代码？](https://medium.com/teamzerolabs/2020-node-js-quick-upgrade-guide-cf671a83f4e7)（英文）
